                        ...
                        for (var i in data) {
                            switch (type) {
                                case Object:
                                    ...
                                    $element = jQuery('<textarea/>', { name: i
                                    }).val(data[i]);
                                    break;
                                case Array:
            ...
            server: new DATA.STORE.Server(this)
                        ...
                        this.meta = {
                            version: {
                                ...
                                update: {
                                key: 'update', value: undefined
                                },
                                revision: {
                                key: 'revision', value: undefined
                                }
                        };
                        }
...
return _this.open();
                    ...
                    Database.prototype.destroy = function (success, failure) {
                        ...
                        try {
                            ...
                            setTimeout(function () {
return MODULE.State.terminate === _this.state() && _this.down(); }, 3000);
                        }
                        catch (err) {
                            this.down();
                            }
                    ...
                    Database.prototype.verify = function (success) {
                        ...
                        meta.get(scheme.revision.key, function (event) {
                            ...
                            if(!data) {
                                meta.set(meta.setBuffer({
                                    key: scheme.revision.key, value: revision
                                }));
                            }
                        ...
                        meta.get(scheme.update.key, function (event) {
                            ...
                            if (!data || !_this.refresh) {
                                meta.set(meta.setBuffer({
                                    key: scheme.update.key, value: days +_this.refresh
                            }));
                                success();
                            }
...
return !!host;
                ...
                Balancer.prototype.bypass = function (setting) {
                    ...
                    var parallel = this.parallel_, hosts = this.chooseServers_(setting), option = jQuery.extend({
                        }, setting.ajax, setting.balance.option.ajax);
                    ...
                    if (cache && cache.jqXHR && 200 === +cache.jqXHR.status) {
                        ...
                        if (this.model_.isDeferrable) {
                            ...
                            var context = jQuery.extend({
                                }, jQuery.ajaxSettings, setting.ajax);
...
return that.binds_[i].responseJSON = that.binds_[i].responseJSON || that.jsons_[i]; });
...
return jQuery(elem).width();
                        ...
                        if (jQuery.when) {
                                        ...
                                        return onready();
                                        })[then](function () { return onrender(); })[then](function () { return onload(); });
                                        }
                                        else {
                                        onready(function () {
                                        return onrender(function () { return onload(); }); });
                        }
                        })
                        ...
                        if (element.hasAttribute('src') ? loadedScripts[element.src]: !this.util_.trim(element.innerHTML)) {
                            ...
                            if (element.hasAttribute('src') && element.getAttribute('src')) {
                                ...
                                if (element.hasAttribute('async')) {
                                    jQuery.ajax(jQuery.extend({
                        }, setting.ajax, setting.load.ajax, {
                                        url: element.src,
                                        dataType: 'script',
                                    async: true,
                                global: false,
...
return defer.resolve([element, data]); },
                                            error: function (err) {
...
mode = this.mode_;
    ...
    (function (MODEL) {
                    ...
                    setTimeout(function () {
return _this.parser.parse('') || _this.model_.disable(); }, 100);
                    }
            ...
            var Main = (function () {
                ...
                Main.prototype.initialize = function ($context, setting) {
                    ...
                    this.data.loadBuffers();
                        setTimeout(function () { return _this.page.landing = null;
                        }, 1500);
                };
...
return swap(i);
            ...
            Main.prototype.overlay = function (setting, immediate) {
                ...
                if (this.isHashChange(setting)) {
                    ...
                    setTimeout(function () {
return _this.app_.data.loadScrollPosition(); }, 1);
                    }
